...
During training, we can achieve 3.1 batches/second, with a batchsize of 64 per GPU. Using 8 gradient accumulation steps (effective batch size of 1024), this gives around 0.39 gradient updates/second. This gives around 33k gradient updates per day, which is not incredible, but sufficient to train a model in a reasonable amount of time. It is much slower than the 5.4 gradient updates per second reported by the REPA paper, but that is to be expected. They have 8x H100s, and we're using 2x RTX3090s.
